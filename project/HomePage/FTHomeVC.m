...
@interface FTHomeVC ()<SDCycleScrollViewDelegate>
...
- (void)viewDidLoad {
    [super viewDidLoad];
    if ([[FT_Defaults stringForKey:MainLeft] isEqualToString:@"2"] && (![CLLocationManager locationServicesEnabled])) {
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [FTCommonObject postData];
}

- (void)aboutgently
{
    [FTNetting getWithURLServiceString:aboutgently parameters:nil success:^(FTResponseModel *model) {
        if (model.success) {
            NSDictionary *dic = (NSDictionary *)model.data;
            [FT_Defaults safe_SetObject:[dic arrayForKey:@"alicent"] forKey:MainCity];
...
{
    WEAK_SELF
    [FTNetting getWithURLServiceString:aboutcollapse parameters:nil success:^(FTResponseModel *model) {
...
- (void)resetSubViewsWith:(NSDictionary *)dic
{
    ...
    topImageView.image = FTImage(@"HomePage_top_large_back_Image");
    ...
    cardImageView.image = FTImage(@"HomePage_top_backLarge_Image");
    ...
    leftTitLabel.font = FTRegularFont(12);
    ...
    [topBtn setBackgroundImage:FTImage(@"login_btn_back_Image") forState:UIControlStateNormal];
    [topBtn.titleLabel setFont:FTBoldFont(18)];
    ...
    if (topLargeArr.count > 0) {
        ...
        UIImageView *leftImageView = [[UIImageView alloc]initWithFrame:CGRectMake((kSCREEN_WIDTH - 164 - 164 - 15)/2, cardImageView.bottom + 45, 164, 80)];
        leftImageView.image = FTImage(@"HomePage_top_left_product_Image");
        [self.s_scrollView addSubview:leftImageView];
        
        UIImageView *rightImageView = [[UIImageView alloc]initWithFrame:CGRectMake(leftImageView.right + 15, leftImageView.top, 164, 80)];
        rightImageView.image = FTImage(@"HomePage_top_right_product_Image");
        [self.s_scrollView addSubview:rightImageView];
        
        UIImageView *image1 = [[UIImageView alloc]initWithFrame:CGRectMake((kSCREEN_WIDTH - 343)/2, leftImageView.bottom + 15, 343, 234)];
        image1.image = FTImage(@"HomePage_Image1");
        ...
        image.image = FTImage(@"HomePage_Image");
        ...
        topImageView.image = FTImage(@"HomePage_top_small_back_Image");
...
//                backView.image = FTImage(@"HomePage_back_Image");
...
//                [productIconImageView sd_setImageWithURL:[NSURL URLWithString:[ssubdic stringForKey:@"latter"]] placeholderImage:FTImage(@"logoImage")];
//                
...
//                productTitLabel.font = FTMediumFont(14);
...
//                [btn.titleLabel setFont:FTBoldFont(16)];
...
//                    titLabel.font = FTRegularFont(12);
...
//                    valueLabel.font = FTMediumFont(16);
...
- (void)aboutlawrencenovember:(NSString *)productIdStr
{
    ...
    [SVProgressHUD show];
    [FTNetting postWithURLServiceString:aboutlawrencenovember parameters:dic success:^(FTResponseModel *model) {
        if (model.success) {
            NSDictionary *dic = (NSDictionary *)model.data;
            NSString *str = [dic stringForKey:@"bribe"];
            [FTCommonObject pushActionWithUrlStr:str];
        }else{
        }
    } failure:^(NSError *error) {
    }];
