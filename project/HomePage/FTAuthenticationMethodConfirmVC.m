//
//  FTAuthenticationMethodConfirmVC.m
//  project
//
...
@interface FTAuthenticationMethodConfirmVC ()
@property (nonatomic, strong)NSMutableArray *s_textfieldArr;

@end

@implementation FTAuthenticationMethodConfirmVC
...
- (void)viewDidLoad {
    ...
    centerImageView.image = FTImage(@"Detail_confirm_Image");
    ...
    tipLabel.font = FTRegularFont(12);
    ...
    for (int k = 0; k < titArr.count; k++) {
        ...
        if (k == 2) {
            [textfield addTapAction:^(UIGestureRecognizer *sender) {
                ...
                FTSelectDateVC *vc = [FTSelectDateVC new];
...
- (void)snowsrhys
{
    ...
    [SVProgressHUD show];
    WEAK_SELF
    [FTNetting postWithURLServiceString:snowsrhys parameters:dic success:^(FTResponseModel *model) {
        STRONG_SELF
