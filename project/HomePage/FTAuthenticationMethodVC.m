...
#import "FTAuthenticationMethodConfirmVC.h"

@interface FTAuthenticationMethodVC ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@implementation FTAuthenticationMethodVC

- (void)viewDidLoad {
    [super viewDidLoad];
    [self initNaviBarWithTitle:@"Authentication" LeftImage:FTImage(@"THblackBackImage") leftTitle:nil rightImage:nil rightTitle:nil delegate:self];
    [self addSubViews];
...
- (void)addSubViews
{
    ...
    [titBtn setImage:FTImage(@"Detail_up_Image") forState:UIControlStateNormal];
    ...
    [titBtn.titleLabel setFont:FTMediumFont(16)];
    ...
    blueTitLabel.font = FTMediumFont(16);
    ...
    iconImageView.image = FTImage(@"Detail_down_Image");
    ...
    for (int k = 0; k < self.m_believesArr.count; k++) {
        ...
        titLabel.font = FTRegularFont(15);
...
{
    [FTCommonObject postDataWithStartTime:self.s_startTimeStr endTime:[FTCommonObject getTimeStampString] type:@"2" order:nil];
    self.s_typeStr = str;
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        [FTNetting uploadImage:img parameters:dic success:^(FTResponseModel *model) {
            ...
            if (model.success) {
                ...
                FTAuthenticationMethodConfirmVC *vc = [FTAuthenticationMethodConfirmVC new];
