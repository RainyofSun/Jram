//
//  SEWodeViewController.m
//  股票18
//
//  Created by 周群 on 2019/11/13.
...
{
    WEAK_SELF
    [FTNetting getWithURLServiceString:snowsarcy parameters:nil success:^(FTResponseModel *model) {
        STRONG_SELF
        if (model.success) {
            NSDictionary *dic = model.data;
            NSDictionary *userInfoDic = [dic dictionaryForKey:@"userInfo"];
            [self.s_headImageView sd_setImageWithURL:[NSURL URLWithString:[userInfoDic stringForKey:@"occurring"]] placeholderImage:FTImage(@"Center_head_Image")];
...
- (void)addSubViews
{
    ...
    backImageView.image = FTImage(@"Center_back_Image");
    ...
    topView.image = FTImage(@"Center_top_Image");
    ...
    self.s_headImageView.image = FTImage(@"Center_head_Image");
    ...
    self.s_idLabel.font = FTBoldFont(20);
    ...
    infoLabel.font = FTRegularFont(15);
    ...
    for (int k = 0; k < titArr.count; k++) {
        ...
        infoLabel.textColor = UIColorFromRGB(MainText3Color);
        infoLabel.textAlignment = NSTextAlignmentCenter;
        infoLabel.font = FTRegularFont(14);
...
- (void)addSubViewWithArr:(NSArray *)arr
{
    ...
    for (int k = 0; k < arr.count; k++) {
        ...
        imageView.contentMode = UIViewContentModeScaleAspectFit;
        [imageView sd_setImageWithURL:[NSURL URLWithString:[dic stringForKey:@"smith"]] placeholderImage:FTImage(@"Detail_icon_Image1")];
        [backView addSubview:imageView];
