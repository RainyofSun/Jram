...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self initNaviBarWithTitle:@"Set up" LeftImage:FTImage(@"THblackBackImage") leftTitle:nil rightImage:nil rightTitle:nil delegate:self];
    [self addSubViews];
...
- (void)addSubViews
{
    ...
    iconImageView.image = FTImage(@"logoImage");
    ...
    versionTitLabel.font = FTRegularFont(16);
    ...
    versionLabel.font = FTRegularFont(16);
...
- (void)signOut
{
    FTSignOutVC *vc = [FTSignOutVC new];
...
{
    FTAccountCancellationVC *vc = [FTAccountCancellationVC new];
    UINavigationController *nc = [[UINavigationController alloc]initWithRootViewController:vc];
    nc.modalPresentationStyle = UIModalPresentationOverCurrentContext;
    [self presentViewController:nc animated:NO completion:nil];
}
