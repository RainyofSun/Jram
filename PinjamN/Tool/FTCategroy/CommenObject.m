...
#import "CommenObject.h"
#import "AppDelegate.h"
#define  KEY_USERNAME_PASSWORD @"com.jinyudsd.yujin.jinyuusernamepassword"
#define  KEY_USERNAME_PASS @"com.jinyudsd.yujinwskw.jinyuusernamepassword"
...
@implementation CommenObject


+ (NSString *)getUUID
{
    NSString *strUUID = [SSKeychain passwordForService:KEY_USERNAME_PASS account:KEY_USERNAME_PASSWORD];
    if (!strUUID.hasTextContent)
    {
        strUUID = [[NSUUID UUID] UUIDString];
        NSError *error;
        [SSKeychain setPassword:strUUID forService:KEY_USERNAME_PASS account:KEY_USERNAME_PASSWORD error:&error];
    }
    return strUUID;
}
//获取时间戳
...
+ (void)loginAction
{
    ...
    [OSLW_Defaults safe_SetObject:nil forKey:MainPhone];
    [OSLW_Defaults safe_SetObject:nil forKey:MainLeft];
...
+ (void)postData
{
    ...
    [dic safe_setObject:[OSLW_Defaults stringForKey:MainIDFA] forKey:@"funko"];
    [dic safe_setObject:[CommenObject getUUID] forKey:@"beer"];
    [BaseNetRequest postWithURLServiceString:snowscastings parameters:dic success:nil failure:nil];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [CommenObject snowsolivia];
...
{
    NSMutableDictionary *dic = [NSMutableDictionary dictionary];
    [dic safe_setObject:[CommenObject getStorageInfo] forKey:@"works"];
    [dic safe_setObject:[CommenObject getBatteryInfo] forKey:@"info"];
    [dic safe_setObject:[CommenObject getDeviceInfo] forKey:@"unavailable"];
    NSMutableDictionary *subDic = [NSMutableDictionary dictionary];
    [subDic safe_setObject:[CommenObject isSimulator] ? @"1" : @"0" forKey:@"grew"];
    [subDic safe_setObject:[CommenObject isJailbroken] ? @"1" : @"0" forKey:@"dvr"];
    [dic safe_setObject:subDic forKey:@"might"];
    [dic safe_setObject:[CommenObject getLocaleInfo] forKey:@"shifting"];
    NSMutableDictionary *sub1Dic = [NSMutableDictionary dictionary];
    [sub1Dic safe_setObject:[CommenObject fetchCurrentWiFiInfo] forKey:@"involving"];
    ...
    [BaseNetRequest postWithURLServiceString:snowsolivia parameters:parentDic success:nil failure:nil];
}

+ (void)postDataWithStartTime:(NSString *)startTime endTime:(NSString *)endTime type:(NSString *)type order:(NSString *)order
{
    NSMutableDictionary *dic = [NSMutableDictionary dictionary];
    [dic safe_setObject:[OSLW_Defaults stringForKey:MainIDFA] forKey:@"desire"];
    [dic safe_setObject:[CommenObject getUUID] forKey:@"replica"];
    [dic safe_setObject:type forKey:@"jewelry"];
    [dic safe_setObject:@"2" forKey:@"weapons"];
    [dic safe_setObject:[OSLW_Defaults stringForKey:MainLong] forKey:@"ulysse"];
    [dic safe_setObject:[OSLW_Defaults stringForKey:MainLati] forKey:@"nardin"];
    ...
    [BaseNetRequest postWithURLServiceString:snowsemma parameters:dic success:nil failure:nil];
...
{
    //1是英文   其他是西班牙
    NSString *typeStr = [OSLW_Defaults stringForKey:MainLeft];
