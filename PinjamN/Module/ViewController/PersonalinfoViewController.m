...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self initNaviBarWithTitle:self.m_titStr LeOSLWImage:OSLWImage(@"THblackBackImage") leftTitle:nil rightImage:nil rightTitle:nil delegate:self];
...
- (void)snowshenley
{
    ...
    [SVProgressHUD show];
    WEAK_SELF
    [BaseNetRequest postWithURLServiceString:snowshenley parameters:dic success:^(BaseResponseModel *model) {
        STRONG_SELF
        if (model.success) {
            [SVProgressHUD dismiss];
...
- (void)snowstoussaint
{
    ...
    [SVProgressHUD show];
    WEAK_SELF
    [BaseNetRequest postWithURLServiceString:snowstoussaint parameters:dic success:^(BaseResponseModel *model) {
        STRONG_SELF
        if (model.success) {
            [SVProgressHUD dismiss];
            NSDictionary *dic = (NSDictionary *)model.data;
...
- (void)addSubViewsWithArr:(NSArray *)arr
{
    ...
    for (int k = 0 ; k < arr.count; k++) {
        ...
        NSAttributedString *accountAttrString = [[NSAttributedString alloc] initWithString:[dic stringForKey:@"confirmed"] attributes:@{NSForegroundColorAttributeName:UIColorFromRGB(0xcccccc),NSFontAttributeName:OSLWRegularFont(16)}];
        ...
        [textfield addTapAction:^(UIGestureRecognizer *sender) {
            ...
            if ([[dic stringForKey:@"animation"] isEqualToString:@"voyagesa"]) {
                ...
                [selectTextfield resignFirstResponder];
                PickerCityViewController *vc = [PickerCityViewController new];
                WEAK_SELF
...
- (void)snowsgeorgie
{
    ...
    WEAK_SELF
    [BaseNetRequest postWithURLServiceString:snowsgeorgie parameters:dic success:^(BaseResponseModel *model) {
        STRONG_SELF
        if (model.success) {
            [SVProgressHUD showSuccessWithStatus:model.msg];
            [CommenObject postDataWithStartTime:self.s_startTimeStr endTime:[CommenObject getTimeStampString] type:@"6" order:nil];
...
- (void)snowssteve
{
    ...
    WEAK_SELF
    [BaseNetRequest postWithURLServiceString:snowssteve parameters:dic success:^(BaseResponseModel *model) {
        STRONG_SELF
        if (model.success) {
            [SVProgressHUD showSuccessWithStatus:model.msg];
            [self.navigationController popViewControllerAnimated:YES];
            [CommenObject postDataWithStartTime:self.s_startTimeStr endTime:[CommenObject getTimeStampString] type:@"5" order:nil];
        }else{
            [SVProgressHUD showInfoWithStatus:model.msg];
        }
    } failure:^(NSError *error) {
