...
@property (nonatomic, strong)EmptyDataView   *s_noDataView;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.s_hasBool = YES;
    [self initNaviBarWithTitle:@"Order" LeOSLWImage:nil leftTitle:nil rightImage:nil rightTitle:nil delegate:self];
    [self addSubViews];
}

- (EmptyDataView *)s_noDataView
{
    if (!_s_noDataView) {
        _s_noDataView = [[EmptyDataView alloc]initWithFrame:self.s_tableView.bounds];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    OrderTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:celldequeuntifier];
    if (!cell) {
        cell = [[OrderTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:celldequeuntifier];
    }
...
- (void)snowssmith
{
    ...
    WEAK_SELF
    [BaseNetRequest postWithURLServiceString:snowssmith parameters:dic success:^(BaseResponseModel *model) {
        STRONG_SELF
...
- (void)addSubViews
{
    ...
    for (int k = 0; k < titArr.count; k++) {
        ...
        [btn.titleLabel setFont:OSLWBoldFont(14)];
