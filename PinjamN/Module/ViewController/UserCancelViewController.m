...
@interface UserCancelViewController ()
...
@implementation UserCancelViewController
...
- (void)addSubViews
{
    ...
    self.s_centerView.image = OSLWImage(@"Center_AccountCancellation_Image");
    ...
    tipLabel.font = OSLWRegularFont(16);
    ...
    [self.s_tipBtn setImage:OSLWImage(@"login_select_nomarl_Image") forState:UIControlStateNormal];
    [self.s_tipBtn setImage:OSLWImage(@"login_select_selected_Image") forState:UIControlStateSelected];
    ...
    [self.s_tipBtn.titleLabel setFont:OSLWRegularFont(14)];
...
- (void)snowsconsidine
{
    ...
    [SVProgressHUD show];
    [BaseNetRequest getWithURLServiceString:snowsconsidine parameters:nil success:^(BaseResponseModel *model) {
        if(model.success){
