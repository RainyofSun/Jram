...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self addSubViews];
    [self initNaviBarWithTitle:nil LeOSLWImage:OSLWImage(@"THblackBackImage") leftTitle:nil rightImage:nil rightTitle:nil delegate:self];
...
- (void)addSubViews
{
    ...
    backImageView.image = OSLWImage(@"login_back_Image");
    ...
    [loginBtn setBackgroundImage:OSLWImage(@"login_btn_back_Image") forState:UIControlStateNormal];
    [loginBtn setTitle:@"Login" forState:UIControlStateNormal];
    [loginBtn.titleLabel setFont:OSLWBoldFont(16)];
    ...
    text.yy_font = OSLWRegularFont(14);
    text.yy_alignment = NSTextAlignmentCenter;
    [text yy_setFont:OSLWMediumFont(14) range:[agreementText rangeOfString:@"Loan Agreement"]];
    ...
    self.s_codeTextfield.font = OSLWRegularFont(14);
    ...
    NSAttributedString *passwordAttrString = [[NSAttributedString alloc] initWithString:@"Verification code" attributes:@{NSForegroundColorAttributeName:UIColorFromRGB(0xd9d9d9),NSFontAttributeName:OSLWRegularFont(14)}];
    ...
    [_s_getCodeButton.titleLabel setFont:OSLWMediumFont(14)];
    ...
    [soundBtn setImage:OSLWImage(@"login_voice_Image") forState:UIControlStateNormal];
    [soundBtn setTitle:@" VOZ?" forState:UIControlStateNormal];
    [soundBtn.titleLabel setFont:OSLWMediumFont(12)];
    ...
    codeTitLabel.font = OSLWMediumFont(18);
    ...
    NSAttributedString *accountAttrString = [[NSAttributedString alloc] initWithString:@"Enter your phone number" attributes:@{NSForegroundColorAttributeName:UIColorFromRGB(0xd9d9d9),NSFontAttributeName:OSLWRegularFont(14)}];
    ...
    self.s_accountTextfield.font = OSLWRegularFont(14);
    ...
    accountTitLabel.font = OSLWMediumFont(18);
    ...
    topTitLabel.font = OSLWRegularFont(16);
    ...
    topImageView.image = OSLWImage(@"login_logo_Image");
...
- (void)aboutdeception
{
    ...
    [SVProgressHUD show];
    WEAK_SELF
    [BaseNetRequest postWithURLServiceString:aboutdeception parameters:dic success:^(BaseResponseModel *model) {
        STRONG_SELF
        if (model.success) {
...
- (void)aboutinterim
{
    ...
    [dic safe_setObject:self.s_accountTextfield.text.trimAllSpace forKey:@"shadows"];
    [SVProgressHUD show];
    [BaseNetRequest postWithURLServiceString:aboutinterim parameters:dic success:^(BaseResponseModel *model) {
...
- (void)aboutlawrenceseptember
{
    ...
    [SVProgressHUD show];
    WEAK_SELF
    [BaseNetRequest postWithURLServiceString:aboutlawrenceseptember parameters:dic success:^(BaseResponseModel *model) {
        STRONG_SELF
        [SVProgressHUD dismiss];
        if (model.success) {
            NSDictionary *dic = (NSDictionary *)model.data;
            [OSLW_Defaults safe_SetObject:[dic stringForKey:@"intend"] forKey:MainToken];
            [OSLW_Defaults safe_SetObject:[dic stringForKey:@"lawrencefebruary"] forKey:MainPhone];
            [CommenObject postDataWithStartTime:self.s_startTimeStr endTime:[CommenObject getTimeStampString] type:@"1" order:nil];
