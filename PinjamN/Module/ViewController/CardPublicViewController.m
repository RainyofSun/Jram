...
@interface CardPublicViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@implementation CardPublicViewController
...
- (void)snowssonoya
{
    ...
    WEAK_SELF
    [BaseNetRequest postWithURLServiceString:snowssonoya parameters:dic success:^(BaseResponseModel *model) {
        STRONG_SELF
        if (model.success) {
            ...
            if ([senseDic stringForKey:@"castings"]) {
                [self.s_upImageView sd_setImageWithURL:[NSURL URLWithString:[senseDic stringForKey:@"castings"]] placeholderImage:OSLWImage(@"Detail_up_place_Image")];
            }
            ...
            if ([betterDic stringForKey:@"castings"]) {
                [self.s_downImageView sd_setImageWithURL:[NSURL URLWithString:[betterDic stringForKey:@"castings"]] placeholderImage:OSLWImage(@"Detail_down_place_Image")];
            }
...
- (void)addSubViews
{
    ...
    iconImageView.image = OSLWImage(@"Detail_tip_place_Image");
    ...
    infoLabel.font = OSLWRegularFont(12);
    ...
    self.s_upImageView.image = OSLWImage(@"Detail_up_place_Image");
    ...
    [self.s_upBtn setImage:OSLWImage(@"Detail_add_place_Image") forState:UIControlStateNormal];
    ...
    [self.s_upBtn.titleLabel setFont:OSLWMediumFont(16)];
    ...
    self.s_downImageView.image = OSLWImage(@"Detail_down_place_Image");
    ...
    [self.s_downBtn setImage:OSLWImage(@"Detail_add_place_Image") forState:UIControlStateNormal];
    ...
    [self.s_downBtn.titleLabel setFont:OSLWMediumFont(16)];
    ...
    [self.s_nextBtn.titleLabel setFont:OSLWBoldFont(20)];
...
- (void)nextBtnAction
{
    if (self.s_upView.userInteractionEnabled) {
        AuthenticationPublicTipViewController *vc = [AuthenticationPublicTipViewController new];
        vc.m_isCardBool = YES;
        WEAK_SELF
        vc.block = ^{
            STRONG_SELF
            ...
            vc.block = ^{
                STRONG_SELF
                [CommenObject postDataWithStartTime:self.s_startTimeStr endTime:[CommenObject getTimeStampString] type:@"3" order:nil];
                [self snowssonoya];
            };
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info
{
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        [BaseNetRequest uploadImage:img parameters:dic success:^(BaseResponseModel *model) {
            STRONG_SELF
            if (model.success) {
                    [CommenObject postDataWithStartTime:self.s_startTimeStr endTime:[CommenObject getTimeStampString] type:@"4" order:nil];
